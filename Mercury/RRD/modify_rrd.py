...
class RRDCalculator:
    ...
    def transform(self, smiles):
        ...
        try:
            ...
            dfa = pd.DataFrame(data=[[0 for _ in range(len(atom_feature_config['maxv'].keys()))]], columns  = atom_feature_config['maxv'].keys(), index=[0])
            dfb = pd.DataFrame(data=[[0 for _ in range(len(bond_feature_config['maxv'].keys()))]], columns  = bond_feature_config['maxv'].keys(), index=[0])
