...
class SmilesDataset(dgl.data.DGLDataset):
    def __init__(self, args, mode, feature_encoder=None, raw_graphs=None):
        ...
        self.label_path = self.path + '_labels.pkl'
    ...
    def save(self):
        ...
        print('saving ' + self.mode + ' label to ' + self.path + '_labels.pkl')
    ...
    def load(self):
        ...
        print('loading ' + self.mode + ' products from ' + self.path + '_labels.pkl')
    ...
    def process(self):
        ...
        for i, (raw_reactant_graph, raw_product_graph, label) in enumerate(self.raw_graphs):
            if i % 10000 == 0:
                print('%dk' % (i // 1000))
    ...
    def has_cache(self):
        return os.path.exists(self.path + '_reactant_graphs.bin') and os.path.exists(self.path + '_product_graphs.bin') and os.path.exists(self.label_path)
...
def read_data(dataset, mode):
    ...
    with open(path) as f:
        for line in f.readlines():
            idx, product_smiles, reactant_smiles, _, label = line.strip().split(',')
            ...
            graphs.append([reactant_graph, product_graph, label])
